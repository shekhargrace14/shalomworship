...
export default function SearchPage() {
  ...
  return (
    <>
      ...
      {results.length > 0 && <h2 className="p-2">Search result of "{query}"</h2>}
      {results.length > 0 && (
        <div className="p-2 rounded-lg bg-muted shadow  w-full sm:w-3/5 ">
          {results.map((r) => (
            <Link key={r.id} href={`/song/${r.slug}`} className="block">
              <div className="px-2 py-2 rounded-md flex gap-3 hover:bg-ring cursor-pointer">
                <Image src={r.image} alt={r.title} width={60} height={60} className="rounded-md object-cover" />
