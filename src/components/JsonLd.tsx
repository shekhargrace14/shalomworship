...
export default async function JsonLd({ id }: { id: string }) {
  ...
  const lyricSnippet = getLyricSnippet(songData, lang) || getLyricSnippet(songData, 'en') || '';
  ...
  const jsonLdData = {
    ...
    description: [
      ...
      songData?.searchVariant[0] ? `This this song is widely known by the refrain "${songData.searchVariant[0]}".` : null,
    ]
      .filter(Boolean)
      .join(' '),
    ...
    workExample: safeVideoId ? { '@type': 'MusicRecording', url: `https://www.youtube.com/watch?v=${safeVideoId}` } : undefined,
