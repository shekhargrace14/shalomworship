...
export default function BottomPlayer() {
  const { audioUrl, title, image, artist, isPlaying, togglePlay, setPlaying } = usePlayerStore();
  ...
  const format = (time: number) => new Date(time * 1000).toISOString().substr(14, 5);
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-black text-white px-4 py-2 shadow-md z-50">
      <div className="grid grid-cols-3 ">
        ...
        <div className=" flex  justify-start items-center gap-4">
          <Image src={image || '/default-image.jpg'} alt={title || 'Song Image'} width={40} height={40} className="bg-gray-800 object-cover h-12 w-12" priority={true} />
          <div>
