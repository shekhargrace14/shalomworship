...
const BookmarkSong = ({ song, setlistId }: BookmarkSongProps) => {
    ...
    const isAddedAnywhere = setlists.some(s =>
        s.songs.some(x => x.id === song.id)
    );
        ...
        return (
            <Dialog>
                <DialogTrigger asChild>
                    <button type="button">
                        <Bookmark
                            className={`cursor-pointer transition-colors ${isAddedAnywhere ? "text-accent" : "text-muted-foreground"
                                }`}
                            stroke="currentColor"
                            fill={isAddedAnywhere ? "currentColor" : "none"}
                        />
                    </button>
                </DialogTrigger>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col">
                                        <p className="text-base text-extrabold">{s.name}</p>
                                        <p className="text-xs"><Badge variant="outline"> {new Date(s.createdAt).toLocaleDateString()}</Badge></p>
                                    ...
                                    <button
                                        type="button"
                                        className={`p-1 transition-colors ${isAdded ? "text-accent" : "text-muted-foreground"
                                            }`}
                                        onClick={() => {
                                            isAdded
                                                ? removeSong(s.id, song.id)
    ...
    return (
        <button
            ...
            onClick={() =>
                isAdded
                    ? removeSong(setlistId, song.id)
                    : addSong(setlistId, song)
            }
        >
            <Bookmark
                className={`cursor-pointer ${isAdded ? "text-accent" : "text-muted-foreground"
                    } ${!ready ? "opacity-50" : ""} ` }
                stroke="currentColor"
                fill={isAdded ? "currentColor" : "none"}
            />
