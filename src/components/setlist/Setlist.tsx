    ...
    return (
        <section className="bg-card/60 rounded-md p-2 items-center">
            <div className='flex justify-between'>
                ...
                <h3 className='text-lg mb-1 capitalize font-semibold'><Link href={`/setlist?id=${setlist.id}`}>{setlist.name}</Link></h3>
                <div className="flex gap-2 w-full mb-4 ">
                    <Badge> <NotebookText /> {setlist.songs.length} song</Badge>
                    <Badge> <CalendarDays /> {new Date(setlist.createdAt).getDate()}</Badge>
                        ...
                        <ul className='list-decimal ml-6'>
                            {setlist.songs.map((song: any) => (
                                <li className=' hover-bg-background' >
                                    <Link key={song.id} href={`/song/${song.slug}`}>{song.title}</Link>
                                </li>
                            ))}
                        </ul>
                                                    </>
