...
import { CalendarDays, Globe, Lock, EyeOff, Clock3, ChevronRight } from 'lucide-react';
      ...
      {variant === 'minimal' ? (
        ...
        <Card className="transition-all hover:border-primary hover:shadow-md cursor-pointer">
          <CardHeader className="">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <CardTitle className="text-3xl line-clamp-1">{setlist.title}</CardTitle>

                {setlist.theme && <p className=" text-xl text-muted-foreground">{setlist.theme}</p>}
              </div>
            </div>
          </CardHeader>

          <CardContent className="space-y-3">
            {setlist.description && <p className="line-clamp-2 text-sm text-muted-foreground">{setlist.description}</p>}
